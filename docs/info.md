...
- Iteration: rephraseOffset = transcript length at last successful rephrase. Empty rephrased pane => fresh pass (<transcript> only). Non-empty => refine pass, transcript split into <transcript> (already incorporated) + <additional_transcript> (spoken since). Persisted to localStorage transcriber:rephrased / transcriber:rephrase-offset, restored by restoreTranscript (same crash-recovery guarantee as the transcript).
- Prompt: REPHRASE_SYSTEM_PROMPT in app.js. NO fixed template (a forced Comments/Action items/Conditions structure was tried first and dropped as too rigid) — the output takes whatever shape fits: one thought = a plain paragraph, several points = a numbered list, a long multi-topic message may use short headings. Nature of each point must be evident from how it reads, not from a bucket label. XML-style tags for the input blocks.
...
- API: postRephrase() — text-only chat call, bypasses resolveModelKind (that probe is only for audio routing). temperature 0.01, reasoning {enabled:false}.
- REASONING-DISABLE, verified live on OpenRouter (2026-07-25) against gemini-3.1-flash-lite AND gpt-5.4-mini: {"enabled":false} and {"effort":"none"} both hold reasoning_tokens at 0. **{"max_tokens":0} must NOT be used — it ENABLED reasoning on gpt-5.4-mini (6214 reasoning tokens).** Both models currently default reasoning OFF, so the param is insurance against that default flipping. Positive control: {"effort":"high"} -> 5807 (gemini) / 18440 (gpt) reasoning tokens.
...
- WHY THIS CANNOT REPEAT-APPLY: the instruction is never persisted anywhere, so a later pass cannot resend it and shrink the text again. Its effect still sticks because a refine pass PRESERVES `<current_rephrased>` rather than regenerating — halve it once and it stays halved. (Left in the transcript it WOULD have replayed forever: a refine pass sends the prior transcript as well as the new slice.)
...
- setRephrased(text, toEnd): scrolls to the bottom on a refine pass or an error append (the new material is there), stays at the top on a fresh pass (you read it from the start).
